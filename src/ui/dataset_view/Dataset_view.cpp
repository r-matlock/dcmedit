...
Dataset_view::Dataset_view()
	...
	connect(m_tree_view, &QTreeView::doubleClicked, this, [this](const QModelIndex& proxy_index) {
		QModelIndex src = m_proxy_model->mapToSource(proxy_index);

		// Tell presenter to open the Edit Value dialog
		element_activated(src);
	});
...
void Dataset_view::show_element_context_menu(const QPoint& pos, const QModelIndex& index)
{
    ...
    Qt::ItemFlags f = m_proxy_model->sourceModel()->flags(index);
    ...
    if (f & Qt::ItemIsEditable) {
        ...
        // Non-editable fields get a disabled note
        QAction* locked = menu->addAction("This field is locked");
