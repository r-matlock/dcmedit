...
void Edit_all_files_presenter::apply() {
    ...
    // Block StudyDate edits/delete before doing anything
    auto lowered = tag_path;
    std::transform(lowered.begin(), lowered.end(), lowered.begin(), ::tolower);
    const bool is_study_date = (lowered.find("0008,0020") != std::string::npos) ||
                               (lowered.find("studydate") != std::string::npos);

    if (is_study_date) {
        m_view.show_error("Not allowed",
            "Editing or deleting StudyDate (0008,0020) is disabled.");
        return;
    }
